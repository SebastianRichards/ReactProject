...
const EditTubing = ({conditionsSetTube, setConditionsSetTube, tubeLength, setTubeLength, tubeDiameter, setTubeDiameter, loops, setLoops, material, setMaterial}) => {
    ...
    const submitHandler = (e) => {
        e.preventDefault();
        if (/^[0-9]+$/.test(details.tubeLength) && /^[0-9]+$/.test(details.tubeDiameter) && /^[0-9]+$/.test(details.loops) && /^[0-9]+$/.test(details.material)) {
            ...
            setError("Please enter integer values in all fields.");
        }
